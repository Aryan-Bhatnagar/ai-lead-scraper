"""
Freelancer opportunity provider adapter.
"""
import asyncio
from typing import List, Dict, Any, Optional
from datetime import datetime

from ..services.freelancer_scraper_service import FreelancerScraperService
...
class FreelancerProvider(BaseOpportunityProvider):
    """Freelancer opportunity provider."""

    def __init__(self, config: dict = None):
        super().__init__("freelancer", config)
        self.scraper_service = FreelancerScraperService()

    async def search_opportunities(self, query: str, limit: int = 100) -> List[Opportunity]:
        """Search for opportunities on Freelancer."""
        ...
        for i, job_data in enumerate(raw_jobs):
            try:
                opportunity = self._normalize_freelancer_job(job_data, f"freelancer-{i}")
                if opportunity:
                    opportunities.append(opportunity)
            except Exception as e:
                # Log error but continue processing other jobs
                print(f"Error normalizing Freelancer job {i}: {e}")
                continue

        return opportunities

    async def get_opportunity_details(self, opportunity_id: str) -> Optional[Opportunity]:
        """Get detailed information about a specific Freelancer opportunity."""
        ...
        try:
            ...
            raw_jobs = await loop.run_in_executor(
                ...
                lambda: self.scraper_service.scrape_jobs(
                    ...
                    max_results=50  # Get more to increase chance of match probability
                )
            )
            ...
            for job_data in raw_jobs:
                ...
                if numeric_id in job_id or numeric_id in job_url:
                    return self._normalize_freelancer_job(job_data, opportunity_id)

            # If no exact match, return the first job as fallback
            if raw_jobs:
                return self._normalize_freelancer_job(raw_jobs[0], opportunity_id)

        except Exception as e:
            print(f"Error fetching Freelancer opportunity details: {e}")

        return None

    def get_supported_categories(self) -> List[str]:
        """Return list of categories supported by Freelancer."""
        return [
            "Web Development", "Mobile Apps", "Software Development", "UI/UX Design",
            "Internet Marketing", "Writing & Translation", "Design & Multimedia",
            "Sales & Marketing", "Admin Support", "Customer Service", "Accounting & Legal",
            "Engineering & Architecture", "Data Science & Analytics", "Finance & Management",
            "Business", "Writing", "Translation", "Graphic Design", "Video & Animation",
            "Music & Audio", "Photography", "Fashion Design", "Architecture & Interior Design"
        ]

    def _normalize_freelancer_job(self, job_data: Dict[str, Any], opp_id: str) -> Optional[Opportunity]:
        """Normalize raw Freelancer job data into Opportunity model."""
        try:
            # Extract basic fields
            title = job_data.get('title', f'Freelancer Job {opp_id}')
            ...
            category = job_data.get('category', 'Software Development')
            skills = job_data.get('skills', ['Web Development'])
            ...
            estimated_value = job_data.get('estimated_value', budget_max * 0.75 if budget_max > 0 else 0.0)

            # Build URL
            url = job_data.get('url', f"https://www.freelancer.com/projects/{opp_id}")
            ...
            print(f"Error normalizing Freelancer job data: {e}")
            return None